...
(function initThree() {
    ...
    Promise.all([
        webgl.texture,
        webgl.loader.load('../../img/planets/slides/earth/2.jpg'),
        //webgl.loader.load('/demo/presents/3.jpg'),
        // webgl.loader.load('/demo/presents/3.jpg'),
        //loadAsync("/demo/presents/v1.mp4"),
    ...
    webgl.texturesOptions = [
        ...
        { index: 1, texture: "image", quote: '“Maybe you have to know the <span style="color:#f4852a">darkness</span> <br>before you can appreciate the light”', threshold: 60, random: 2.0, depth: 4.0, size: 1.0, square: 0 },
...
function onMouseMove(event) {
    webgl.mouse.x = ((event.clientX - webgl.container.getBoundingClientRect().left) / webgl.renderer.domElement.offsetWidth) * 2 - 1;
    webgl.mouse.y = - ((event.clientY - webgl.container.getBoundingClientRect().top) / (webgl.renderer.domElement.offsetHeight)) * 2 + 1;
...
function changeTexture(e) {
    ...
    if (Date.now() - webgl.lastClick < 1000) return;
    ...
    if (opt.a1) {
        ...
        if (opt.a10) {
            webgl.texturaAnimation1 = gsap.set(webgl.particlesMesh.material.uniforms.uTexture, { value: webgl.loader.load("/demo/presents/20cc.jpg"), delay: 4 })
		}
	}
}
